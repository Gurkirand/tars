...
def test():
    ...
    features = np.reshape(features, (10, 8, 2))
    labels = np.reshape(labels, (10, 8, 1))
    
    rnn = RNN_cell(2, 16, 1)

    # input = tf.placeholder(tf.float32, [10, 8, 2])
    # output = tf.placeholder(tf.float32, [10, 8, 1])
    # train = rnn.train(input, output, 10, 1, 0.1)
    rnn.train(features, labels, 10, 2, 0.1)
